...
public class ControladorProvedor {
    ...
    public void eliminarProvedor() {
        ...
        if (fila == -1) {
            JOptionPane.showMessageDialog(vista, "Debe seleccionar un proveedor de la tabla");
            return;
        }
        ...
        if (confirmacion == JOptionPane.YES_OPTION) {
            if (provedorDAO.eliminar(id)) {
                ...
                JOptionPane.showMessageDialog(vista, "Proveedor desactivado exitosamente");
            } else {
                JOptionPane.showMessageDialog(vista, "Error al desactivar el proveedor", "Error", JOptionPane.ERROR_MESSAGE);
            }
        }
    }
    ...
    public void mostrarTodosProvedores() {
        ArrayList<Provedor> provedores = provedorDAO.obtenerTodos();
        DefaultTableModel modelo = (DefaultTableModel) vista.getTablaProvedores().getModel();
        modelo.setRowCount(0);
    ...
    private boolean validarDatos(Provedor provedor) {
        ...
        if (!provedor.getTelefono().isEmpty() && !validarTelefono(provedor.getTelefono())) {
            JOptionPane.showMessageDialog(vista, "El formato del teléfono no es válido (Debe ser ####-####)");
            return false;
        }

        if (provedor.getEmail().isEmpty() && provedor.getDireccion().isEmpty()) {
