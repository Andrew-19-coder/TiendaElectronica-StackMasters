...
public class ControladorCliente {
    ...
    private boolean validarDatos(Cliente cliente) {
        ...
        if (!validarCedula(cliente.getCedula())) {
            JOptionPane.showMessageDialog(vista,
                    "Cédula inválida, debe ser una cédula de Costa Rica",
                    "Validación",
                    JOptionPane.WARNING_MESSAGE);
            return false;
        }
        ...
        String nombre = cliente.getNombreCompleto().trim();
        if (nombre.split("\\s+").length < 2) {
            JOptionPane.showMessageDialog(vista,
                    "Debe ingresar el nombre completo (nombre y apellidos)",
                    "Validación",
                    JOptionPane.WARNING_MESSAGE);
            return false;
        }
    ...
    private boolean validarCedula(String cedula) {
        return cedula.matches("^[1-5]0\\d{3}0\\d{3}$");
    }
